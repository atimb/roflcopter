...
%% Sensor constants
gyro_const = 1; % Modifier, for tuning
accel_const = 1;
longterm_const = 1;

gyro_bandwidth = 100; % Hz
...
engine_varspeed = 2; % Newton / second varying speed

%% Starting engine rates
e_ = zeros(4,1);
e_(1) = 2.5;
e_(2) = 2.5;
e_(3) = 2.5;
...
longterm = zeros(3,1); % Autopilot variable
    ...
    if (autopilot_on)
        ...
        if (samplecounter == samplerate)
            ...
            if (i >= 1/h)
                longterm_ = zeros(3,1);
                for j=i-1/h:i
                    longterm_(1:2) = longterm_(1:2) + o(1:2,i);
                    longterm_(3) = longterm_(3) + w(3,i);
                end
                longterm(:) = 1.0*longterm(:) + 0.1*longterm_(:)*h;
            end
            e_(1) = 2.5 - gyro(3)*gyro_const - gyro(1)*gyro_const - accel(1)*accel_const - longterm(1)*longterm_const - longterm(3)*longterm_const;
            e_(2) = 2.5 + gyro(3)*gyro_const - gyro(2)*gyro_const - accel(2)*accel_const - longterm(2)*longterm_const + longterm(3)*longterm_const;
            e_(3) = 2.5 - gyro(3)*gyro_const + gyro(1)*gyro_const + accel(1)*accel_const + longterm(1)*longterm_const - longterm(3)*longterm_const;
            e_(4) = 2.5 + gyro(3)*gyro_const + gyro(2)*gyro_const + accel(2)*accel_const + longterm(2)*longterm_const + longterm(3)*longterm_const;
        end
    end
...
%% 3D plot: animation of quadrotor
